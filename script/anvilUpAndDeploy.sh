...
anvil  --block-time 1 --port 8545 --accounts 10 > "$LOG_FILE" 2>&1 & 
...
forge script ./DeployContracts.s.sol --slow -g 200 --gas-limit 80000000 --via-ir --tc DeployContracts --broadcast --rpc-url=http://localhost:8545 --json --private-key 0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d | grep '^{.*}$' | jq 'del(.gas_used, .returns)' > ./output/addresses.json
