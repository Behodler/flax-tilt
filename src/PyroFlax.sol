...
import {PyroERC20} from "./PyroERC20.sol";
...
import {SafeERC20} from "@oz_tilt/contracts/token/ERC20/utils/SafeERC20.sol";

// Explanation: Identical to original PyroFlax ERC20 but renamed to avoid clashes with openzeppelin

/// @author Justin Goro
/** @notice In order to re-use audited code, the PyroFlax 3 code is copied over. Unlike prior PyroTokens, this one isn't connected to Behodler and so doesn't reference the
 ...
 */
contract PyroFlax is PyroERC20, ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        ISuperfluid host
    ) Ownable(msg.sender) PyroERC20(name_, symbol_) SuperfluidToken(host) {
